...
export class UpdateDeviceDto {
  ...
  @ApiProperty({ description: 'Тип датчика' })
  readonly sensor_type_id: number;

  @ApiProperty({ description: 'Характер выходного сигнала' })
  readonly output_signal_id: number;

  @ApiProperty({ description: 'Технология изготовления' })
  readonly manufacturing_technology_id: number;

  @ApiProperty({ description: 'Характер преобразования сигнала' })
  readonly signal_conversation_id: number;
  ...
  @ApiProperty({ description: 'Частотная характеристика (изображение)' })
  readonly dynamic_frequency_response: string;
